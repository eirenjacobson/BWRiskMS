...
\texttt{M1} fitted a spatial model of $P(\text{GVP})$ to data collected prior to the onset of naval training activity. This model used a MRF smooth to account for the spatial structure of the range and a spline on depth, with an offset for the log of the area effectively monitored by each hydrophone. Both the MRF and spline on depth were significant at the $\alpha$ = 0.05 level (\textit{p}-value < 2E-16), indicating that GVPs varied space. The model explained 13.5\% of deviance in the dataset, and visual inspection of observed versus predicted values indicated a good fit to the data (Fig. \@ref(fig:m1obspred)). The model \texttt{M1} predicted highest $P(\text{GVP})$ at hydrophone depths between 1,500 and 2,000 m (Fig. \@ref(fig:sdepth)).
