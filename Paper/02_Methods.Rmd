...
\end{equation}
for each $\texttt{M} = \texttt{M1}$, $\texttt{M2}$, and $\texttt{M3}$. Then, we calculated the change in $P(\text{GVP})$ for each set of covariates between $\texttt{M3}$ and $\texttt{M1}$ ($\Delta_{M3:M1}$) and between $\texttt{M3}$ and $\texttt{M2}$ ($\Delta_{M3:M2}$) for each realization of the posterior simulation. 
\begin{align}
...
\end{align}
For each received level we calcualted the 2.5th, 50th, and 97.5th quantiles of $\Delta_{M3:M1}$ and $\Delta_{M3:M2}$ to create 95\% CIs of change in $P(\text{GVP})$ across possible received levels. 
